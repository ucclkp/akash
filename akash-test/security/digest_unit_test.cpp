...
namespace test {
    ...
    { "SHA1", digest::SHAVersion::SHA1, digest::SHA1::kHashSize,
      {
        /* 1 */ { TEST1, length(TEST1), 1, 0, 0, "A9993E364706816ABA3E25717850C26C9CD0D89D" },
        /* 2 */ { TEST2_1, length(TEST2_1), 1, 0, 0, "84983E441C3BD26EBAAE4AA1F95129E5E54670F1" },
        /* 3 */ { TEST3, length(TEST3), 1000000, 0, 0, "34AA973CD4C4DAA4F61EEB2BDBAD27316534016F" },
        /* 4 */ { TEST4, length(TEST4), 10, 0, 0, "DEA356A2CDDD90C7A7ECEDC5EBB563934F460452" },
        /* 5 */ { "", 0, 0, 0x98, 5, "29826B003B906E660EFF4027CE98AF3531AC75BA" },
        /* 6 */ { "\x5e", 1, 1, 0, 0, "5E6F80A34A9798CAFC6A5DB96CC57BA4C4DB59C2" },
        /* 7 */ { TEST7_1, length(TEST7_1), 1, 0x80, 3, "6239781E03729919C01955B3FFA8ACB60B988340" },
        /* 8 */ { TEST8_1, length(TEST8_1), 1, 0, 0, "82ABFF6605DBE1C17DEF12A394FA22A82B544A35" },
        /* 9 */ { TEST9_1, length(TEST9_1), 1, 0xE0, 3, "8C5B2A5DDAE5A97FC7F9D85661C672ADBF7933D4" },
        /* 10 */{ TEST10_1, length(TEST10_1), 1, 0, 0, "CB0082C8F197D260991BA6A460E76E202BAD27B3" }
      }, SHA1_SEED, {
    ...
    { "SHA224", digest::SHAVersion::SHA224, digest::SHA224::kHashSize,
      {
        /* 1 */ { TEST1, length(TEST1), 1, 0, 0, "23097D223405D8228642A477BDA255B32AADBCE4BDA0B3F7E36C9DA7" },
        /* 2 */ { TEST2_1, length(TEST2_1), 1, 0, 0, "75388B16512776CC5DBA5DA1FD890150B0C6455CB4F58B1952522525" },
        /* 3 */ { TEST3, length(TEST3), 1000000, 0, 0, "20794655980C91D8BBB4C1EA97618A4BF03F42581948B2EE4EE7AD67" },
        /* 4 */ { TEST4, length(TEST4), 10, 0, 0, "567F69F168CD7844E65259CE658FE7AADFA25216E68ECA0EB7AB8262" },
        /* 5 */ { "", 0, 0, 0x68, 5, "E3B048552C3C387BCAB37F6EB06BB79B96A4AEE5FF27F51531A9551C" },
        /* 6 */ { "\x07", 1, 1, 0, 0, "00ECD5F138422B8AD74C9799FD826C531BAD2FCABC7450BEE2AA8C2A" },
        ...
        /* 9 */ { TEST9_224, length(TEST9_224), 1, 0xE0, 3, "54BEA6EAB8195A2EB0A7906A4B4A876666300EEFBD1F3B8474F9CD57" },
    ...
    { "SHA256", digest::SHAVersion::SHA256, digest::SHA256::kHashSize,
      {
    ...
    { "SHA384", digest::SHAVersion::SHA384, digest::SHA384::kHashSize,
      {
    ...
    { "SHA512", digest::SHAVersion::SHA512, digest::SHA512::kHashSize,
      {
        /* 1 */ { TEST1, length(TEST1), 1, 0, 0,
    ...
    struct hmachash {
      ...
      { /* 1 */ {
        ...
        "3A126854"
            ...
            digest::SHA1::kHashSize,
            digest::SHA224::kHashSize,
            digest::SHA256::kHashSize,
            digest::SHA384::kHashSize,
            digest::SHA512::kHashSize
        } },
      { /* 2 */ {
...
{
    for (; len-- > 0; str++)
        putchar(isprint(static_cast<unsigned char>(*str)) ? *str : '.');
}
...
void printResult(
    ...
    const char *hashname, const char *testtype, const std::string& testname,
    const char *resultarray)
{
    if (resultarray) {
        int ret = checkmatch(Message_Digest, resultarray, hashsize);
        LOG(Log::INFO) << hashname << " " << testtype << " " << testname.c_str() << ": " <<
            (ret ? "PASSED" : "FAILED");
    }
}
 ...
 */
int hash(
    int testno, int loopno, int hashno,
    ...
    const char *resultarray, int hashsize)
{
    ...
    digest::HKDF hkdf;
    int err, i;
    uint8_t Message_Digest_Buf[digest::USHA::kMaxHashSize];
    ...
    if (info)
        Message_Digest = new uint8_t[okmlen];

    memset(&sha, '\343', sizeof(sha)); /* force bad data into struct */
    memset(&hmac, '\343', sizeof(hmac));
    memset(&hkdf, '\343', sizeof(hkdf));

    err = info ? hkdf.init(hashes[hashno].whichSha, keyarray, keylen) :
        keyarray ? hmac.init(hashes[hashno].whichSha, keyarray, keylen) :
        sha.init(hashes[hashno].whichSha);
    if (err != digest::shaSuccess) {
    ...
    for (i = 0; i < repeatcount; ++i) {
        err = info ? hkdf.update(reinterpret_cast<const uint8_t *>(testarray), length) :
            keyarray ? hmac.update(reinterpret_cast<const uint8_t *>(testarray), length) :
            sha.update(reinterpret_cast<const uint8_t *>(testarray), length);
        if (err != digest::shaSuccess) {
    ...
    if (numberExtrabits > 0) {
        err = info ? hkdf.finalBits(extrabits, numberExtrabits) :
            keyarray ? hmac.finalBits(uint8_t(extrabits), numberExtrabits) :
            sha.finalBits(uint8_t(extrabits), numberExtrabits);
        if (err != digest::shaSuccess) {
    ...
    err = info ? hkdf.result(nullptr, info, infolen, Message_Digest, okmlen) :
        keyarray ? hmac.result(Message_Digest) :
    ...
    auto buf = std::to_string(testno + 1);
...
{
    unsigned char prk[digest::USHA::kMaxHashSize + 1];
    uint8_t okm[255 * digest::USHA::kMaxHashSize + 1];
    ...
    int err = digest::HKDF::calculate(
        hkdfhashes[testno].which_sha,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].saltarray),
        hkdfhashes[testno].saltlength,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].ikmarray),
        hkdfhashes[testno].ikmlength,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].infoarray),
    ...
    printResult(okm, hkdfhashes[testno].okmlength,
        digest::USHA::USHAHashName(hkdfhashes[testno].which_sha), "hkdf standard test",
        buf.str(), hkdfhashes[testno].okmarray);

    /* Now run hkdfExtract() by itself against the test vectors */
    /* to verify the intermediate results. */
    err = digest::HKDF::hkdfExtract(hkdfhashes[testno].which_sha,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].saltarray),
        hkdfhashes[testno].saltlength,
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].ikmarray),
    ...
    buf.clear();
    buf << "hkdfExtract " << (testno + 1);
    printResult(prk, digest::USHA::USHAHashSize(hkdfhashes[testno].which_sha),
        digest::USHA::USHAHashName(hkdfhashes[testno].which_sha), "hkdf standard test",
        buf.str(), hkdfhashes[testno].prkarray);

    /* Now run hkdfExpand() by itself against the test vectors */
    /* using the intermediate results from hkdfExtract. */
    err = digest::HKDF::hkdfExpand(hkdfhashes[testno].which_sha, prk,
        digest::USHA::USHAHashSize(hkdfhashes[testno].which_sha),
        reinterpret_cast<const uint8_t *>(hkdfhashes[testno].infoarray),
 ...
 */
void randomtest(
    int hashno, const char *seed, int hashsize,
    const char **resultarrays, int randomcount)
{
    int i;
    unsigned char SEED[digest::USHA::kMaxHashSize], MD[1003][digest::USHA::kMaxHashSize];
    ...
    for (int j = 0; j < randomcount; j++) {
        ...
        for (i = 3; i < 1003; i++) {
            ...
            Mi.init(hashes[hashno].whichSha);
            Mi.update(MD[i - 3], hashsize);
            Mi.update(MD[i - 2], hashsize);
            Mi.update(MD[i - 1], hashsize);
            /* MDi = SHA(Mi); */
            Mi.result(MD[i]);
        }
        /* MDj = Seed = MDi; */
        ...
        /* OUTPUT: MDj */
        auto buf = std::to_string(j);
        printResult(SEED, hashsize, hashes[hashno].name, "random test",
            buf, resultarrays ? resultarrays[j] : nullptr);
    }
}
...
void testErrors() {
    digest::USHA usha;
    uint8_t Message_Digest[digest::USHA::kMaxHashSize];
    int err = 0;

    for (int hashno = int(digest::SHAVersion::SHA1);
        hashno <= int(digest::SHAVersion::SHA512); ++hashno)
    {
        memset(&usha, '\343', sizeof(usha)); /* force bad data */
        usha.init(static_cast<digest::SHAVersion>(hashno));
        usha.result(Message_Digest);
        usha.update(reinterpret_cast<const uint8_t*>("foo"), 3);

        LOG(Log::INFO) << "\nError " << err << ". Should be " << digest::shaStateError << ".";
        LOG(Log::INFO) << hashes[hashno].name << " se: " <<
            ((err == digest::shaStateError) ? "PASSED" : "FAILED");

        err = usha.finalBits(0x80, 3);
        LOG(Log::INFO) << "\nError " << err << ". Should be " << digest::shaStateError << ".";
        LOG(Log::INFO) << hashes[hashno].name << " se: " <<
            ((err == digest::shaStateError) ? "PASSED" : "FAILED");
    }
...
int TEST_SHA()
{
    ...
    {
        printf("Hash %s\n", hashes[hashno].name);
        int err = digest::shaSuccess;

        for (int loopno = 1; (loopno <= 5) && (err == digest::shaSuccess); ++loopno) {
            for (int testno = 0; (testno < TESTCOUNT) && (err == digest::shaSuccess); ++testno) {
                err = hash(
            ...
            for (int testno = 0; (testno < HMACTESTCOUNT) && (err == digest::shaSuccess); ++testno) {
                err = hash(testno, loopno, hashno,
                    ...
                    1, 0, 0,
                    reinterpret_cast<const unsigned char *>(hmachashes[testno].keyarray[hashno]
            ...
            for (int testno = 0; (testno < HKDFTESTCOUNT) && (err == digest::shaSuccess); ++testno) {
...
int TEST_MD5() {
    ubassert(digest::MD5::cal("") == "D41D8CD98F00B204E9800998ECF8427E");
    ubassert(digest::MD5::cal("", 1) == "D41D8CD98F00B204E9800998ECF8427E");
    ubassert(digest::MD5::cal("a") == "0CC175B9C0F1B6A831C399E269772661");
    ubassert(digest::MD5::cal("a", 2) == "0CC175B9C0F1B6A831C399E269772661");
    ubassert(digest::MD5::cal("12345678901234567890123456789012345678901234567890123456") == "49F193ADCE178490E34D1B3A4EC0064C");
    ubassert(digest::MD5::cal("12345678901234567890123456789012345678901234567890123456", 1) == "49F193ADCE178490E34D1B3A4EC0064C");
    ubassert(digest::MD5::cal("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", 63) == "D174AB98D277D9F5A5611C2C9F419D9F");
    ubassert(digest::MD5::cal("12345678901234567890123456789012345678901234567890123456789012345678901234567890") == "57EDF4A22BE3C955AC49DA2E2107B67A");
    ubassert(digest::MD5::cal("12345678901234567890123456789012345678901234567890123456789012345678901234567890", 1) == "57EDF4A22BE3C955AC49DA2E2107B67A");
